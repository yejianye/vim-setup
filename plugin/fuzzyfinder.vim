...
function! s:GetUserDefinedFiles()
  let user_files = readfile(g:FuzzyFinderOptions.UserFile.filename)
  return sort(filter(map(user_files, 'fnamemodify(v:val, '':p'')'), 'filereadable(v:val)'))
endfunction

function! s:GetLocalTags(filename)
	let debug = []
	let lines = split(system(g:FuzzyFinderOptions.TagInFile.ctags . ' -f - ' . a:filename), '\n')
	let taginfo = {}
	for line in lines
		let debug += [line]
		let tagname = matchstr(line, '^[^\t]*')
		let debug += [tagname]
		let pos = matchstr(line, '/\^.*\$/')
		let pos = substitute(pos, '*','\\*','g')
		let pos = substitute(pos, '^/','', '')
		let pos = substitute(pos, '/$','', '')
		let debug += [pos]
		let taginfo[tagname] = pos
	endfor

	"call writefile(debug, 'vim_debug.txt')
	return taginfo
endfunction
...
function! g:FuzzyFinderMode.Base.launch_base(initial_pattern, partial_matching)
  ...
  for [lhs, rhs] in [
        ...
        \   [ self.key_complete   , self.to_str('on_cr(' . s:OPEN_MODE_CURRENT . ', 0)') ],
...
" OBJECT: g:FuzzyFinderMode.TagInFile ----------------------------------------- {{{1
let g:FuzzyFinderMode.TagInFile = copy(g:FuzzyFinderMode.Base)

"
function! g:FuzzyFinderMode.TagInFile.on_complete(base)
  let patterns = self.make_pattern(a:base)
  let stats = self.get_filtered_stats(a:base)
  let result = self.find_tag(patterns.re, s:SuffixNumber(patterns.base), self.enumerating_limit)
  return map(result, 's:SetRanks(v:val, v:val.word, a:base, stats)')
...
"
function! g:FuzzyFinderMode.TagInFile.on_open(expr, mode)
  if a:mode == s:OPEN_MODE_SPLIT
	  execute ':sp'
  elseif a:mode == s:OPEN_MODE_VSPLIT
	  execute ':vertical sp'
  endif
  let line = search(self.cache[expand('%')].taginfo[a:expr],'nc')
  execute 'normal '.line.'G'
endfunction

"
function! g:FuzzyFinderMode.TagInFile.on_mode_enter_pre()
  let self.tag_files = s:GetCurrentTagFiles()
endfunction

"
function! g:FuzzyFinderMode.TagInFile.find_tag(pattern, index, limit)
  let key = g:FuzzyFinderMode.TagInFile.filename
  " cache not created or tags file updated? 
  call extend(self, { 'cache' : {} }, 'keep')
  if !exists('self.cache[key]') || getftime(key) >= self.cache[key].time
    echo 'Caching tag list...'
    let taginfo = s:GetLocalTags(key)
    let items = s:MapToSetSerialIndex(map(keys(taginfo), '{ "word" : v:val }'), 1)
    let self.cache[key] = { 'time'  : localtime(), 'items' : items, 'taginfo' : taginfo }
  endif
  echo 'Filtering tag list...'
  let result = s:FilterMatching(self.cache[key].items, 'word', a:pattern, a:index, a:limit)
  return map(result, 's:SetFormattedWordToAbbr(v:val, self.max_menu_width)')
...
" OBJECT: g:FuzzyFinderMode.UserFile ---------------------------------- {{{1
let g:FuzzyFinderMode.UserFile = copy(g:FuzzyFinderMode.Base)

"
function! g:FuzzyFinderMode.UserFile.on_complete(base)
  let patterns = self.make_pattern(a:base)
  let base_tail = s:SplitPath(a:base).tail
  let stats = self.get_filtered_stats(a:base)
  echo 'Making user file list...'
  let result = self.find_user_file(patterns.re, s:SuffixNumber(patterns.base), self.enumerating_limit)
  return map(result, 's:SetRanks(v:val, s:SplitPath(matchstr(v:val.word, ''^.*[^/\\]'')).tail, base_tail, stats)')
endfunction

"
function! g:FuzzyFinderMode.UserFile.on_open(expr, mode)
  call s:OpenFile(a:expr, a:mode, self.reuse_window)
endfunction

"
function! g:FuzzyFinderMode.UserFile.find_user_file(pattern, index, limit)
  let key = g:FuzzyFinderOptions.UserFile.filename
  if !len(key)
    return []
  endif
  " cache not created or tags file updated? 
  call extend(self, { 'cache' : {} }, 'keep')
  if !exists('self.cache[key]') || (getftime(key) >= self.cache[key].time)
    echo 'Caching user-file list...'
    "let items = s:Unique(s:Concat(map(copy(self.user_files), 's:GetTaggedFileList(v:val)')))
    let items = s:MapToSetSerialIndex(map(s:GetUserDefinedFiles(), '{ "word" : v:val }'), 1)
    let self.cache[key] = { 'time'  : localtime(), 'items' : items }
  endif
  echo 'Filtering user-file list...'
  call map(self.cache[key].items, 's:ModifyWordAsFilename(v:val, '':.'')')
  let result = s:FilterMatching(self.cache[key].items, 'word', a:pattern, a:index, a:limit)
  return s:MapToSetAbbrWithFileWord(result, self.max_menu_width)
endfunction
...
let g:FuzzyFinderOptions = { 'Base':{}, 'Buffer':{}, 'File':{}, 'Dir':{},
      ...
      \                      'Tag':{}, 'TagInFile':{}, 'TaggedFile':{}, 'UserFile':{},
      \                      'GivenFile':{}, 'GivenDir':{}, 'GivenCmd':{},
      \                      'CallbackFile':{}, 'CallbackItem':{}, }
"-----------------------------------------------------------------------------
let g:FuzzyFinderOptions.Base.key_open           = '<CR>'
let g:FuzzyFinderOptions.Base.key_complete       = '<TAB>'
...
"-----------------------------------------------------------------------------
let g:FuzzyFinderOptions.TagInFile.mode_available   = 1
let g:FuzzyFinderOptions.TagInFile.prompt           = '>TagCurFile>'
let g:FuzzyFinderOptions.TagInFile.prompt_highlight = 'Question'
let g:FuzzyFinderOptions.TagInFile.smart_bs         = 0
let g:FuzzyFinderOptions.TagInFile.switch_order     = 75
"-----------------------------------------------------------------------------
...
"-----------------------------------------------------------------------------
let g:FuzzyFinderOptions.UserFile.mode_available   = 1
let g:FuzzyFinderOptions.UserFile.prompt           = '>UserFile>'
let g:FuzzyFinderOptions.UserFile.prompt_highlight = 'Question'
let g:FuzzyFinderOptions.UserFile.smart_bs         = 0
let g:FuzzyFinderOptions.UserFile.switch_order     = 15
let g:FuzzyFinderOptions.UserFile.reuse_window     = 1
let g:FuzzyFinderOptions.UserFile.filename		   = '~/Documents/file_list'
"-----------------------------------------------------------------------------
...
command! -bang -narg=? -complete=tag    FuzzyFinderTagInFile				 call g:FuzzyFinderMode.TagInFile.launch_base(<q-args>, len(<q-bang>))
...
command! -bang -narg=? -complete=file   FuzzyFinderUserFile                  call g:FuzzyFinderMode.UserFile.launch_base(<q-args>, len(<q-bang>))
